describe('die Telefon- oder Handynummer falsch eingeben, um zu sehen, ob eine Exception geworfen wird.', function() {

  beforeAll(function() {
    console.log('Test Case: Exception Throw, wenn Telefon- oder Handynummer falsch?');
  });

  afterAll(function() {
    console.log('\nDer Bewerbungsablauf wurde abgebrochen, da die Telefon- oder Handynummer falsch eingegeben worden ist. (Erfolgreicher Test)\n');
  });
  ...
  it('die Telefon- oder Handynummer eingeben', function() {
    element(by.name('telefonnummer')).sendKeys('0172832!+-');
    browser.sleep(6000);
  });
  ...
  it('sind Exceptions aufgetreten?', function() {
    expect(browser.getCurrentUrl()).toEqual("http://localhost/bewerberportal.html?absenden=ungueltigetelefonnummer");
  });
});
